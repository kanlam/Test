...
public class FireWorldBMControl : MonoBehaviour {
	...
	void AnimationControl()
	{
		...
		if (staminaBar.value > 0 && isSprinting ||isBootsed) { // ||isbooted
	...
	void Sprinting(){
		...
		if (OnGround && !isSwimming && Input.GetKey (KeyCode.W) || Input.GetKey (KeyCode.S)) {
			isRunning = true;
		} else {
			isRunning = false;
		}
	...
	{
		if (!isDead && other.gameObject.tag == "attack") {
			Health = Health - 1;
			source.clip = Sizzling;
			source.Play ();

			if(Health<=0)
			{
				isDead = true;
				BGMcontrol.source.clip = BGMcontrol.DeadBGM;
				BGMcontrol.source.Play();
			}
		}
		if (other.gameObject.name == "Monster"||other.gameObject.tag=="Lava")
		{
			isDead = true;
			if (isDead)
			{
		...
		if (other.gameObject.name == "FireWorldGoal")
			isWin = true;
		if (isWin)
		{
			BGMcontrol.source.clip = BGMcontrol.WinBGM;
			BGMcontrol.source.Play();
		}
